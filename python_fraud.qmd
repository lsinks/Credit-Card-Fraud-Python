...
plt.clf()
fig, ax = sns.ecdfplot(data = fraud, x = "category", stat = 'proportion')
...
fraud.value_counts("category", sort = True).plot(kind = "bar")
plt.xticks(rotation=45)
plt.show()
```
...
```{python}
fraud_dummies = pd.get_dummies(fraud, columns = ["category"])
...
X = fraud_dummies[feature_cols] # Features
